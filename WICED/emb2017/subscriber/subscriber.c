...
void application_start( void )
{
    ...
    // EMB2017
    wiced_rtos_init_semaphore(&i2cSemaphore);
//    wiced_rtos_create_thread(&i2cThreadHandle, 10, "i2cThread", i2cThread, 1024, NULL);
    ...
    do
    {
        ...
        wiced_i2c_init_tx_message(&i2cmsg, tx_buffer, 2, RETRIES, DISABLE_DMA);

        while ( 1 )
        {
        	wiced_rtos_get_semaphore(&i2cSemaphore, WICED_WAIT_FOREVER);
            WPRINT_APP_INFO(("Position = %d\n",position));
            tx_buffer[1]= (uint8_t)position;
            wiced_i2c_transfer(&i2cDevice, &i2cmsg, NUM_MESSAGES);
        }
